...
class TestScenario:
        ...
        for i, log in enumerate(self.logs, 1):
            try:
                ...
                test_result = TestResult(
                    ...
                    was_detected=is_anomaly,
                    confidence=result.get('confidence', 0.85),
                    criticality=result.get('criticality', 'basse'),
                    actions_suggested=result.get('actions', []),
                ...
                print(f"{symbol} [{i}/{len(self.logs)}] Temps: {response_time:.3f}s | "
                      f"Détecté: {is_anomaly} | "
                      f"Criticité: {result.get('criticality', 'N/A')}")
                
                time.sleep(0.2)  # Délai entre les requêtes
    ...
    def _analyze_log(self, log: str) -> Dict:
        ...
        # Extraire le résultat du format API
        data = response.json()
        if 'result' in data:
            return data['result']
        else:
